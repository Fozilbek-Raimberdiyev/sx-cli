...
export function generateMigration(entityName: { apiIdSingular: string, apiIdPlural: string }, fields: { name: string, type: string }[], projectPath: string,) {
    const migrationPath = path.join(projectPath, "database", "migrations", generateMigrationFileName(entityName.apiIdPlural));
    ...
    public function up()
    {
        Schema::create(
            ...
            function (Blueprint $table) {
                $table->id();
                ${fields.map(field => ` $table->${field.type}('${field.name}');`).join('\n')}
            }
        );
    }
